...
run aws --profile wasabi s3 sync /var/backups/postgresql s3://"{{ wasabi_bucket }}/postgresql/" --no-follow-symlinks --endpoint-url=https://s3.wasabisys.com 2>&1 1>/dev/null && logger -t wasabi "$now" "$source" WASABI PSQL BACKUP SUCCESS || logger -t wasabi "$source" WASABI PSQL BACKUP ERROR
